...
const searchLine = (line) => {
  ...
  for (const [key, value] of Object.entries(replacements)) {
    temp.push(...line.matchAll(new RegExp(key, "g")), ...line.matchAll(new RegExp(value, "g")));
  }
...
return line.replace(pattern, match => replacements[match.toLowerCase()]);
}
...
const getResult = (input) => {
  ...
  const getNumbericCharArray = (str) => str.match(/\d/g)

  lineArray.forEach(row => {
  	const convertedRow = searchLine(row);
    const line = convertLine(convertedRow)
    
    console.log(line)
  
    if (line.length > 0) {
      const array = getNumbericCharArray(line);
      ...
      const firstDigit = Number.parseInt(array[0]);
      const lastDigit = Number.parseInt(array[array.length - 1]);
      const numberString = Number.parseInt(`${firstDigit}${lastDigit}`);
      sum += numberString;
    }
  })
  console.log(sum)
...
/* const convertedString = testCase.replace(pattern, match => replacements[match.toLowerCase()]); */
