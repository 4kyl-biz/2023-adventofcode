 ...
 * Solution
 * 
 * 1. Convert inputString into a 2D array
 * 2. Find the coordinate of numbers, create a key-value pair where key is the number and value is the coordinate
 * 3. For each coordinate of number, detect whether a symbol is around it
 * 4. Save the valid numbers
...
const convertToArray = (inputString) => {
    return inputString.split('').filter(str => str !== '\n');
};
const convertTo2DArray = (array) => {
    let result = [];
    const numberOfColumns = Math.sqrt(array.length);
...
// 3. For each coordinate of number, detect whether a symbol is around it
const isValidCoordinate = (coordinate, map) => {
    const isValid = true;
