...
// Solution
// 1. Read input
// - for each game, split inputString into id, 3 sets of game
// - create a resultMap that contains the game id, highest number of each colour

// 2. Verify result
// - loop through the resultMap, check possibility of each game
// - Add up the game ids to get the result
...
const convertStringToGameSet = (gameSetsString, result) => {
    const gameSetArray = gameSetsString.split(', ');
    for (const item of gameSetArray) {
        const [numberString, colour] = item.split(' ');
        const number = Number.parseInt(numberString);
        if (result[colour] < number) {
...
const convertStringToGame = (gameString) => {
    const result = {
        green: 0,
        blue: 0,
        red: 0
    };
    const [_id, gameSet] = gameString.split(': ');
    const gameSetArray = gameSet.split('; ');
    
    for (const item of gameSetArray) {
        convertStringToGameSet(item, result)
    }

    return result
};

const convertInputToGames = (inputString) => {
    // {blue: number, red: number, green: number}[]
    const games = [];
    
    const gamesString = inputString.split('\n');
    
    games.push(...gamesString.filter(str => !!str).map(convertStringToGame));
...
const isValidGame = (game) => {
    ...
    for (const [colour, number] of Object.entries(game)) {
...
const getResult = (inputString) => {
    ...
    const games = convertInputToGames(inputString);
...
const output = getResult(testCase);
console.log(output);